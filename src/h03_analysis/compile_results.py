...
def get_args():
    ...
    # Data
    parser.add_argument("--ud-path", type=str, default='data/ud/ud-treebanks-v2.5/')
...
def get_representation_results(checkpoint_path, task, language, model, representation):
    ...
    # Fix hack for wrong headers
    if 'shuffle_input' not in df.columns:
        df = df.reset_index()
        df.columns = [
            'run', 'hidden_size', 'nlayers', 'dropout', 'embedding_size',
            'alpha', 'max_rank', 'shuffle_labels', 'shuffle_input'] + \
            df.columns[9:].tolist()
...
def get_lang_results(checkpoint_path, task, lang):
    dfs = []
    if lang == 'english':
        representations = ['bert', 'fast', 'onehot', 'random', 'albert', 'roberta']
    else:
        representations = ['bert', 'fast', 'onehot', 'random']

    for model in ['linear', 'mlp', 'rank-max']:
